...
public class Evento {
   ...
   // @Transient
    ...
    public Atuendo getSugerencia() {
        return this.sugerencias.stream().filter(a -> a.obtenerEstadoAtuendo().estaAceptado()).collect(Collectors.toList()).get(0);
    }

    public long getIdSugerenciaAceptada() {
        return this.getSugerencia().getId();
    }
    ...
    public List<Atuendo> obtenerSugerenciasSinEstado() {
        return this.obtenerSugerencias().stream().filter(atuendo -> atuendo.obtenerEstadoAtuendo().esNuevo()).collect(Collectors.toList());
    }
